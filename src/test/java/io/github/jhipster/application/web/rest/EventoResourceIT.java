...
class EventoResourceIT {
  ...
  private static final Instant UPDATED_TS_EVENTO = Instant
    .now()
  ...
  void getAllEventos() throws Exception {
    ...
    restEventoMockMvc
      ...
      .andExpect(
        jsonPath("$.[*].idEvento").value(hasItem(DEFAULT_ID_EVENTO.intValue()))
      )
      .andExpect(
        jsonPath("$.[*].tsEvento").value(hasItem(DEFAULT_TS_EVENTO.toString()))
      )
  ...
  void getAllEventosWithEagerRelationshipsIsEnabled() throws Exception {
    when(eventoServiceMock.findAllWithEagerRelationships(any()))
  ...
  void deleteEvento() throws Exception {
    ...
    restEventoMockMvc
      .perform(
        delete(ENTITY_API_URL_ID, evento.getId())
