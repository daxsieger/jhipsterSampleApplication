...
class StatoResourceIT {
  ...
  private static final Instant DEFAULT_TS_CAMBIO_STATO = Instant.ofEpochMilli(
    0L
  );
  private static final Instant UPDATED_TS_CAMBIO_STATO = Instant
    .now()
  ...
  void getAllStatoes() throws Exception {
    ...
    restStatoMockMvc
      ...
      .andExpect(
        jsonPath("$.[*].idStadio").value(hasItem(DEFAULT_ID_STADIO.intValue()))
      )
  ...
  void getStato() throws Exception {
    ...
    restStatoMockMvc
      ...
      .andExpect(
        jsonPath("$.tsCambioStato").value(DEFAULT_TS_CAMBIO_STATO.toString())
      );
  }
  ...
  void deleteStato() throws Exception {
    ...
    restStatoMockMvc
      .perform(
        delete(ENTITY_API_URL_ID, stato.getId())
