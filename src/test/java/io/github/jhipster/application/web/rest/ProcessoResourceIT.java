...
class ProcessoResourceIT {
  ...
  void getAllProcessos() throws Exception {
    ...
    restProcessoMockMvc
      ...
      .andExpect(
        jsonPath("$.[*].id").value(hasItem(processo.getId().intValue()))
      )
      .andExpect(
        jsonPath("$.[*].idProcesso")
      ...
      .andExpect(
        jsonPath("$.[*].dsProcesso").value(hasItem(DEFAULT_DS_PROCESSO))
      );
  }
  ...
  void putNewProcesso() throws Exception {
    ...
    // Update the processo
    Processo updatedProcesso = processoRepository
  ...
  void deleteProcesso() throws Exception {
    ...
    restProcessoMockMvc
      .perform(
        delete(ENTITY_API_URL_ID, processo.getId())
