...
public interface EventoRepository extends JpaRepository<Evento, Long> {
  ...
  @Query(
    "select distinct evento from Evento evento left join fetch evento.statis"
  )
  List<Evento> findAllWithEagerRelationships();

  @Query(
    "select evento from Evento evento left join fetch evento.statis where evento.id =:id"
  )
