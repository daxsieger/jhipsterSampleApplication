...
export class StadioService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/stadios");
  ...
  create(stadio: IStadio): Observable<EntityResponseType> {
    return this.http.post<IStadio>(this.resourceUrl, stadio, {
      observe: "response",
    });
  }

  update(stadio: IStadio): Observable<EntityResponseType> {
    return this.http.put<IStadio>(
      `${this.resourceUrl}/${getStadioIdentifier(stadio) as number}`,
      stadio,
      { observe: "response" }
    );
  }
  ...
  partialUpdate(stadio: IStadio): Observable<EntityResponseType> {
    return this.http.patch<IStadio>(
      `${this.resourceUrl}/${getStadioIdentifier(stadio) as number}`,
      stadio,
      { observe: "response" }
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IStadio>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IStadio[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  addStadioToCollectionIfMissing(
    stadioCollection: IStadio[],
    ...stadiosToCheck: (IStadio | null | undefined)[]
  ): IStadio[] {
    const stadios: IStadio[] = stadiosToCheck.filter(isPresent);
    if (stadios.length > 0) {
      const stadioCollectionIdentifiers = stadioCollection.map(
        (stadioItem) => getStadioIdentifier(stadioItem)!
      );
      const stadiosToAdd = stadios.filter((stadioItem) => {
        const stadioIdentifier = getStadioIdentifier(stadioItem);
        if (
          stadioIdentifier == null ||
          stadioCollectionIdentifiers.includes(stadioIdentifier)
        ) {
          return false;
        }
