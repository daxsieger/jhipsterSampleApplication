...
export class StadioComponent implements OnInit {
  ...
  trackId(index: number, item: IStadio): number {
    return item.id!;
  }

  delete(stadio: IStadio): void {
    const modalRef = this.modalService.open(StadioDeleteDialogComponent, {
      size: "lg",
      backdrop: "static",
    });
    modalRef.componentInstance.stadio = stadio;
