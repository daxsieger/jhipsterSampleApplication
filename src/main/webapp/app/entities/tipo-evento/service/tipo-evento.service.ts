...
export class TipoEventoService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/tipo-eventos");
  ...
  create(tipoEvento: ITipoEvento): Observable<EntityResponseType> {
    return this.http.post<ITipoEvento>(this.resourceUrl, tipoEvento, {
      observe: "response",
    });
  }

  update(tipoEvento: ITipoEvento): Observable<EntityResponseType> {
    return this.http.put<ITipoEvento>(
      `${this.resourceUrl}/${getTipoEventoIdentifier(tipoEvento) as number}`,
      tipoEvento,
      { observe: "response" }
    );
  }
  ...
  partialUpdate(tipoEvento: ITipoEvento): Observable<EntityResponseType> {
    return this.http.patch<ITipoEvento>(
      `${this.resourceUrl}/${getTipoEventoIdentifier(tipoEvento) as number}`,
      tipoEvento,
      { observe: "response" }
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<ITipoEvento>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<ITipoEvento[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  addTipoEventoToCollectionIfMissing(
    ...
    if (tipoEventos.length > 0) {
      const tipoEventoCollectionIdentifiers = tipoEventoCollection.map(
        (tipoEventoItem) => getTipoEventoIdentifier(tipoEventoItem)!
      );
      const tipoEventosToAdd = tipoEventos.filter((tipoEventoItem) => {
        ...
        if (
          tipoEventoIdentifier == null ||
          tipoEventoCollectionIdentifiers.includes(tipoEventoIdentifier)
        ) {
          return false;
        }
