...
export class ProduttoreComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.produttoreService
      ...
      .subscribe({
        next: (res: HttpResponse<IProduttore[]>) => {
  ...
  trackId(index: number, item: IProduttore): number {
    return item.id!;
  }

  delete(produttore: IProduttore): void {
    const modalRef = this.modalService.open(ProduttoreDeleteDialogComponent, {
      size: "lg",
      backdrop: "static",
    });
    modalRef.componentInstance.produttore = produttore;
  ...
  protected onSuccess(
    ...
    this.page = page;
    if (navigate) {
      this.router.navigate(["/produttore"], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
