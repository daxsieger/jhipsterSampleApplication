...
export class ProduttoreService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/produttores");
  ...
  create(produttore: IProduttore): Observable<EntityResponseType> {
    return this.http.post<IProduttore>(this.resourceUrl, produttore, {
      observe: "response",
    });
  }

  update(produttore: IProduttore): Observable<EntityResponseType> {
    return this.http.put<IProduttore>(
      `${this.resourceUrl}/${getProduttoreIdentifier(produttore) as number}`,
      produttore,
      { observe: "response" }
    );
  }
  ...
  partialUpdate(produttore: IProduttore): Observable<EntityResponseType> {
    return this.http.patch<IProduttore>(
      `${this.resourceUrl}/${getProduttoreIdentifier(produttore) as number}`,
      produttore,
      { observe: "response" }
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IProduttore>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IProduttore[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  addProduttoreToCollectionIfMissing(
    ...
    if (produttores.length > 0) {
      const produttoreCollectionIdentifiers = produttoreCollection.map(
        (produttoreItem) => getProduttoreIdentifier(produttoreItem)!
      );
      const produttoresToAdd = produttores.filter((produttoreItem) => {
        ...
        if (
          produttoreIdentifier == null ||
          produttoreCollectionIdentifiers.includes(produttoreIdentifier)
        ) {
          return false;
        }
