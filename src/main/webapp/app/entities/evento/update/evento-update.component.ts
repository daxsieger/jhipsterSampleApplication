...
export class EventoUpdateComponent implements OnInit {
  ...
  protected subscribeToSaveResponse(
    result: Observable<HttpResponse<IEvento>>
  ): void {
  ...
  protected updateForm(evento: IEvento): void {
    this.editForm.patchValue({
      ...
      tsEvento: evento.tsEvento
        ? evento.tsEvento.format(DATE_TIME_FORMAT)
        : null,
      note: evento.note,
    ...
    this.assistitosCollection =
      this.assistitoService.addAssistitoToCollectionIfMissing(
        this.assistitosCollection,
        evento.assistito
      );
    this.tiposCollection =
      this.tipoEventoService.addTipoEventoToCollectionIfMissing(
        this.tiposCollection,
        evento.tipo
      );
    this.gestoresCollection =
      this.gestoreService.addGestoreToCollectionIfMissing(
        this.gestoresCollection,
        evento.gestore
      );
    this.produttoresSharedCollection =
      this.produttoreService.addProduttoreToCollectionIfMissing(
        this.produttoresSharedCollection,
        evento.origine
      );
    this.statoesSharedCollection =
      this.statoService.addStatoToCollectionIfMissing(
        this.statoesSharedCollection,
        ...(evento.statis ?? [])
      );
  }
  ...
  protected loadRelationshipsOptions(): void {
    this.assistitoService
      ...
      .pipe(
        map((assistitos: IAssistito[]) =>
          this.assistitoService.addAssistitoToCollectionIfMissing(
            assistitos,
            this.editForm.get("assistito")!.value
          )
        )
      )
      .subscribe(
        (assistitos: IAssistito[]) => (this.assistitosCollection = assistitos)
      );
    ...
    this.tipoEventoService
      ...
      .subscribe(
        (tipoEventos: ITipoEvento[]) => (this.tiposCollection = tipoEventos)
      );
    ...
    this.gestoreService
      ...
      .pipe(
        map((gestores: IGestore[]) =>
          this.gestoreService.addGestoreToCollectionIfMissing(
            gestores,
            this.editForm.get("gestore")!.value
          )
        )
      )
      .subscribe(
        (gestores: IGestore[]) => (this.gestoresCollection = gestores)
      );
    ...
    this.produttoreService
      ...
      .pipe(
        map((produttores: IProduttore[]) =>
          this.produttoreService.addProduttoreToCollectionIfMissing(
            produttores,
            this.editForm.get("origine")!.value
          )
        )
      )
      .subscribe(
        (produttores: IProduttore[]) =>
          (this.produttoresSharedCollection = produttores)
      );
    ...
    this.statoService
      ...
      .pipe(
        map((statoes: IStato[]) =>
          this.statoService.addStatoToCollectionIfMissing(
            statoes,
            ...(this.editForm.get("statis")!.value ?? [])
          )
        )
      )
      .subscribe(
        (statoes: IStato[]) => (this.statoesSharedCollection = statoes)
      );
  }
  ...
  protected createFromForm(): IEvento {
    return {
      ...
      tsEvento: this.editForm.get(["tsEvento"])!.value
        ? dayjs(this.editForm.get(["tsEvento"])!.value, DATE_TIME_FORMAT)
        : undefined,
