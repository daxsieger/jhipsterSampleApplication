...
export class EventoService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/eventos");
  ...
  update(evento: IEvento): Observable<EntityResponseType> {
    ...
    return this.http
      .put<IEvento>(
        `${this.resourceUrl}/${getEventoIdentifier(evento) as number}`,
        copy,
        { observe: "response" }
      )
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  ...
  addEventoToCollectionIfMissing(
    eventoCollection: IEvento[],
    ...eventosToCheck: (IEvento | null | undefined)[]
  ): IEvento[] {
    const eventos: IEvento[] = eventosToCheck.filter(isPresent);
    if (eventos.length > 0) {
      const eventoCollectionIdentifiers = eventoCollection.map(
        (eventoItem) => getEventoIdentifier(eventoItem)!
      );
      const eventosToAdd = eventos.filter((eventoItem) => {
        const eventoIdentifier = getEventoIdentifier(eventoItem);
        if (
          eventoIdentifier == null ||
          eventoCollectionIdentifiers.includes(eventoIdentifier)
        ) {
          return false;
        }
  ...
  protected convertDateFromClient(evento: IEvento): IEvento {
    return Object.assign({}, evento, {
      tsEvento: evento.tsEvento?.isValid()
        ? evento.tsEvento.toJSON()
        : undefined,
    });
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.tsEvento = res.body.tsEvento
        ? dayjs(res.body.tsEvento)
        : undefined;
    }
    return res;
  }
