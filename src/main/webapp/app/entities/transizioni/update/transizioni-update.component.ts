...
export class TransizioniUpdateComponent implements OnInit {
  ...
  protected subscribeToSaveResponse(
    result: Observable<HttpResponse<ITransizioni>>
  ): void {
  ...
  protected updateForm(transizioni: ITransizioni): void {
    ...
    this.processosSharedCollection =
      this.processoService.addProcessoToCollectionIfMissing(
        this.processosSharedCollection,
  ...
  protected loadRelationshipsOptions(): void {
    this.processoService
      ...
      .pipe(
        map((processos: IProcesso[]) =>
          this.processoService.addProcessoToCollectionIfMissing(
            processos,
            this.editForm.get("processo")!.value
          )
        )
      )
      .subscribe(
        (processos: IProcesso[]) => (this.processosSharedCollection = processos)
      );
    ...
    this.stadioService
      ...
      .subscribe(
        (stadios: IStadio[]) => (this.stadiosSharedCollection = stadios)
      );
  }
