...
describe("Transizioni Management Update Component", () => {
  ...
  describe("ngOnInit", () => {
    it("Should call Processo query and add missing value", () => {
      ...
      jest
        .spyOn(processoService, "query")
        .mockReturnValue(of(new HttpResponse({ body: processoCollection })));
      const additionalProcessos = [processo];
      const expectedCollection: IProcesso[] = [
        ...additionalProcessos,
        ...processoCollection,
      ];
      jest
        .spyOn(processoService, "addProcessoToCollectionIfMissing")
      ...
      expect(
        processoService.addProcessoToCollectionIfMissing
      ).toHaveBeenCalledWith(processoCollection, ...additionalProcessos);
    ...
    it("Should call Stadio query and add missing value", () => {
      ...
      jest
        .spyOn(stadioService, "query")
        .mockReturnValue(of(new HttpResponse({ body: stadioCollection })));
      const additionalStadios = [stadioIniziale, stadioFinale];
      const expectedCollection: IStadio[] = [
        ...additionalStadios,
        ...stadioCollection,
      ];
      jest
        .spyOn(stadioService, "addStadioToCollectionIfMissing")
        .mockReturnValue(expectedCollection);
      ...
      expect(stadioService.addStadioToCollectionIfMissing).toHaveBeenCalledWith(
        stadioCollection,
        ...additionalStadios
      );
