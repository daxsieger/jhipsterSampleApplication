...
export class TransizioniService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/transizionis");
  ...
  create(transizioni: ITransizioni): Observable<EntityResponseType> {
    return this.http.post<ITransizioni>(this.resourceUrl, transizioni, {
      observe: "response",
    });
  }

  update(transizioni: ITransizioni): Observable<EntityResponseType> {
    return this.http.put<ITransizioni>(
      `${this.resourceUrl}/${getTransizioniIdentifier(transizioni) as number}`,
      transizioni,
      { observe: "response" }
    );
  }
  ...
  partialUpdate(transizioni: ITransizioni): Observable<EntityResponseType> {
    return this.http.patch<ITransizioni>(
      `${this.resourceUrl}/${getTransizioniIdentifier(transizioni) as number}`,
      transizioni,
      { observe: "response" }
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<ITransizioni>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<ITransizioni[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  addTransizioniToCollectionIfMissing(
    ...
    if (transizionis.length > 0) {
      const transizioniCollectionIdentifiers = transizioniCollection.map(
        (transizioniItem) => getTransizioniIdentifier(transizioniItem)!
      );
      const transizionisToAdd = transizionis.filter((transizioniItem) => {
        ...
        if (
          transizioniIdentifier == null ||
          transizioniCollectionIdentifiers.includes(transizioniIdentifier)
        ) {
          return false;
        }
