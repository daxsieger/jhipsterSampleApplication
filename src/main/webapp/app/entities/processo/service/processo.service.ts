...
export class ProcessoService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/processos");
  ...
  create(processo: IProcesso): Observable<EntityResponseType> {
    return this.http.post<IProcesso>(this.resourceUrl, processo, {
      observe: "response",
    });
  }

  update(processo: IProcesso): Observable<EntityResponseType> {
    return this.http.put<IProcesso>(
      `${this.resourceUrl}/${getProcessoIdentifier(processo) as number}`,
      processo,
      { observe: "response" }
    );
  }
  ...
  partialUpdate(processo: IProcesso): Observable<EntityResponseType> {
    return this.http.patch<IProcesso>(
      `${this.resourceUrl}/${getProcessoIdentifier(processo) as number}`,
      processo,
      { observe: "response" }
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IProcesso>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IProcesso[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  addProcessoToCollectionIfMissing(
    processoCollection: IProcesso[],
    ...processosToCheck: (IProcesso | null | undefined)[]
  ): IProcesso[] {
    const processos: IProcesso[] = processosToCheck.filter(isPresent);
    if (processos.length > 0) {
      const processoCollectionIdentifiers = processoCollection.map(
        (processoItem) => getProcessoIdentifier(processoItem)!
      );
      const processosToAdd = processos.filter((processoItem) => {
        const processoIdentifier = getProcessoIdentifier(processoItem);
        if (
          processoIdentifier == null ||
          processoCollectionIdentifiers.includes(processoIdentifier)
        ) {
          return false;
        }
