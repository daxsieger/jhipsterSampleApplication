...
export class GestoreService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/gestores");
  ...
  create(gestore: IGestore): Observable<EntityResponseType> {
    return this.http.post<IGestore>(this.resourceUrl, gestore, {
      observe: "response",
    });
  }

  update(gestore: IGestore): Observable<EntityResponseType> {
    return this.http.put<IGestore>(
      `${this.resourceUrl}/${getGestoreIdentifier(gestore) as number}`,
      gestore,
      { observe: "response" }
    );
  }
  ...
  partialUpdate(gestore: IGestore): Observable<EntityResponseType> {
    return this.http.patch<IGestore>(
      `${this.resourceUrl}/${getGestoreIdentifier(gestore) as number}`,
      gestore,
      { observe: "response" }
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IGestore>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IGestore[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  addGestoreToCollectionIfMissing(
    gestoreCollection: IGestore[],
    ...gestoresToCheck: (IGestore | null | undefined)[]
  ): IGestore[] {
    const gestores: IGestore[] = gestoresToCheck.filter(isPresent);
    if (gestores.length > 0) {
      const gestoreCollectionIdentifiers = gestoreCollection.map(
        (gestoreItem) => getGestoreIdentifier(gestoreItem)!
      );
      const gestoresToAdd = gestores.filter((gestoreItem) => {
        const gestoreIdentifier = getGestoreIdentifier(gestoreItem);
        if (
          gestoreIdentifier == null ||
          gestoreCollectionIdentifiers.includes(gestoreIdentifier)
        ) {
          return false;
        }
