...
export class AssistitoService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/assistitos");
  ...
  create(assistito: IAssistito): Observable<EntityResponseType> {
    return this.http.post<IAssistito>(this.resourceUrl, assistito, {
      observe: "response",
    });
  }

  update(assistito: IAssistito): Observable<EntityResponseType> {
    return this.http.put<IAssistito>(
      `${this.resourceUrl}/${getAssistitoIdentifier(assistito) as number}`,
      assistito,
      { observe: "response" }
    );
  }
  ...
  partialUpdate(assistito: IAssistito): Observable<EntityResponseType> {
    return this.http.patch<IAssistito>(
      `${this.resourceUrl}/${getAssistitoIdentifier(assistito) as number}`,
      assistito,
      { observe: "response" }
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IAssistito>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IAssistito[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  addAssistitoToCollectionIfMissing(
    ...
    if (assistitos.length > 0) {
      const assistitoCollectionIdentifiers = assistitoCollection.map(
        (assistitoItem) => getAssistitoIdentifier(assistitoItem)!
      );
      const assistitosToAdd = assistitos.filter((assistitoItem) => {
        const assistitoIdentifier = getAssistitoIdentifier(assistitoItem);
        if (
          assistitoIdentifier == null ||
          assistitoCollectionIdentifiers.includes(assistitoIdentifier)
        ) {
          return false;
        }
