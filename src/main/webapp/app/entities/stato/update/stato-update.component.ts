...
export class StatoUpdateComponent implements OnInit {
  ...
  protected subscribeToSaveResponse(
    result: Observable<HttpResponse<IStato>>
  ): void {
  ...
  protected updateForm(stato: IStato): void {
    this.editForm.patchValue({
      ...
      tsCambioStato: stato.tsCambioStato
        ? stato.tsCambioStato.format(DATE_TIME_FORMAT)
        : null,
      stadio: stato.stadio,
    });

    this.stadiosCollection = this.stadioService.addStadioToCollectionIfMissing(
      this.stadiosCollection,
      stato.stadio
    );
  }
      ...
      .pipe(
        map((stadios: IStadio[]) =>
          this.stadioService.addStadioToCollectionIfMissing(
            stadios,
            this.editForm.get("stadio")!.value
          )
        )
      )
