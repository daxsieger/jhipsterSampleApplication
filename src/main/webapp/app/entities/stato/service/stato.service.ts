...
export class StatoService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/statoes");
  ...
  update(stato: IStato): Observable<EntityResponseType> {
    ...
    return this.http
      .put<IStato>(
        `${this.resourceUrl}/${getStatoIdentifier(stato) as number}`,
        copy,
        { observe: "response" }
      )
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  ...
  addStatoToCollectionIfMissing(
    statoCollection: IStato[],
    ...statoesToCheck: (IStato | null | undefined)[]
  ): IStato[] {
    const statoes: IStato[] = statoesToCheck.filter(isPresent);
    if (statoes.length > 0) {
      const statoCollectionIdentifiers = statoCollection.map(
        (statoItem) => getStatoIdentifier(statoItem)!
      );
      const statoesToAdd = statoes.filter((statoItem) => {
        const statoIdentifier = getStatoIdentifier(statoItem);
        if (
          statoIdentifier == null ||
          statoCollectionIdentifiers.includes(statoIdentifier)
        ) {
          return false;
        }
  ...
  protected convertDateFromClient(stato: IStato): IStato {
    return Object.assign({}, stato, {
      tsCambioStato: stato.tsCambioStato?.isValid()
        ? stato.tsCambioStato.toJSON()
        : undefined,
    });
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.tsCambioStato = res.body.tsCambioStato
        ? dayjs(res.body.tsCambioStato)
        : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(
    ...
    if (res.body) {
      res.body.forEach((stato: IStato) => {
        stato.tsCambioStato = stato.tsCambioStato
          ? dayjs(stato.tsCambioStato)
          : undefined;
      });
    }
    return res;
